...
class USSDMakePaymentTest extends TestCase
{
   ...
   public function _test_airtel(): void
   {
      ...
      $urlPrefix = 'swasco';
      $sessionId = '100005060';
      $response = $this->get('/'.$urlPrefix.'/airtel?MSISDN=260977787659&SUBSCRIBER_INPUT=5757&SESSION_ID='.$sessionId.'&IS_NEW_REQUEST=1');
      $response->assertStatus(200);

      $response = $this->get('/'.$urlPrefix.'/airtel?MSISDN=260977787659&SUBSCRIBER_INPUT=1&SESSION_ID='.$sessionId.'&IS_NEW_REQUEST=0');
      $response->assertStatus(200);

      $response = $this->get('/'.$urlPrefix.'/airtel?MSISDN=260977787659&SUBSCRIBER_INPUT=LIV0005559&SESSION_ID='.$sessionId.'&IS_NEW_REQUEST=0');
      $response->assertStatus(200);
