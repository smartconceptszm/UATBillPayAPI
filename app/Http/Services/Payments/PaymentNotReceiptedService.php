   ...
   public function findAll(array $criteria):array|null
   {
      ...
      try {
         ...
         $records = DB::table('payments as p')
                        ...
                        ->select('p.*','pps.shortName as paymentProvider','s.customerJourney',
                                       'm.prompt as paymentType','cw.paymentMethod');
         if($dto->dateFrom && $dto->dateTo){
            $records =$records->where('p.created_at', '>=', $dto->dateFrom)
                              ->where('p.created_at', '<=',$dto->dateTo);
         }
         $records = $records->whereIn('p.paymentStatus', [PaymentStatusEnum::NoToken->value,
