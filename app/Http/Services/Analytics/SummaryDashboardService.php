...
class SummaryDashboardService 
{
   ...
   public function findAll(array $criteria)
   {
      ...
      try {
         $dto = (object)$criteria;
         $user = Auth::user(); 
         if($paymentsSummary = \json_decode(Cache::get($user->id."_summaryDashboard".$dto->theMonth),\json_encode([]), true)){
            ...
            //Get all in Date Range
               ...
               if($thePayments->isNotEmpty()){
                  ...
                  Cache::put($user->id."_summaryDashboard".$dto->theMonth,\json_encode($paymentsSummary),Carbon::now()->addMinutes(3));
                  return $paymentsSummary;
               }else{
                  return [];
               }
